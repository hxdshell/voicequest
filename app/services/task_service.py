...
from app.schema.schema import TaskCreate, TaskIntent
...
class TaskService:
    ...
    def handle_intent(self, user_id: int, intent: TaskIntent) -> dict:
        match intent.intent:
            ...
            case "complete_task":
                return self._handle_complete(user_id, intent)
            case "delay_task":
                return self._handle_delay(user_id, intent)
            case "cancel_task":
                return self._handle_cancel(user_id, intent)
            case _:
                raise ValueError(f"Unknown intent: {intent.intent}")

    def _handle_create(self, user_id: int, intent: TaskIntent) -> dict:
        if not intent.date_time:
            raise ValueError("A due date/time is required to create a task.")

        # utc = parse_utc(iso_string=intent.date_time)
    ...
    def _handle_delay(self, user_id: int, intent: TaskIntent) -> dict:
        if not intent.date_time:
    ...
    def _handle_cancel(self, user_id: int, intent: TaskIntent) -> dict:
        task = self._resolve_task(user_id, intent.task_keyword)
